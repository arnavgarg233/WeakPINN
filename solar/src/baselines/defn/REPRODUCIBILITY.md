...
`<WORKSPACE>/WeakPINN/solar/src/baselines/defn/defn/train.py`

(Same code is mirrored in the working tree at
`<WORKSPACE>/flare-pinn/src/baselines/defn/defn/train.py`.)

### Input feature table
`<WORKSPACE>/flare-pinn/data/defn/defn_features.parquet`

### Per-seed prediction npz files (probabilities + labels)
Directory: `<WORKSPACE>/flare-pinn/outputs/baselines/defn/`
